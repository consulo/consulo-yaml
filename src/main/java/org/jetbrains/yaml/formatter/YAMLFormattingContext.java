...
class YAMLFormattingContext {
    ...
    public final @Nonnull CodeStyleSettings mySettings;
    private final @Nonnull PsiFile myFile;
    private final @Nonnull SpacingBuilder mySpaceBuilder;

    /**
     * This alignments increase partial reformatting stability in case of initially incorrect indents
     */
    private final @Nonnull Map<ASTNode, Alignment> myChildIndentAlignments = FactoryMap.create(node -> Alignment.createAlignment(true));

    private final @Nonnull Map<ASTNode, Alignment> myChildValueAlignments = FactoryMap.create(node -> Alignment.createAlignment(true));
    ...
    YAMLFormattingContext(@Nonnull CodeStyleSettings settings, @Nonnull PsiFile file) {
        mySettings = settings;
        myFile = file;
    ...
    private static boolean isAfterKey(ASTNode node) {
        List<ASTNode> nodes = Stream.iterate(node, TreeUtil::prevLeaf)
            .skip(1)
            .dropWhile(n -> NON_SIGNIFICANT_TOKENS_BEFORE_TEMPLATE.contains(n.getElementType()))
            .limit(2)
            .toList();
    ...
    private static boolean isAfterSequenceMarker(ASTNode node) {
        List<ASTNode> nodes = Stream.iterate(node, Objects::nonNull, ASTNode::getTreePrev)
            .skip(1)
            .filter(n -> !YAMLElementTypes.SPACE_ELEMENTS.contains(n.getElementType()))
            .takeWhile(n -> !YAMLTokenTypes.EOL.equals(n.getElementType()))
            .limit(2)
            .toList();
    ...
    @Nullable
    Spacing computeSpacing(@Nonnull Block parent, @Nullable Block child1, @Nonnull Block child2) {
    ...
    private static @Nullable IElementType getNodeElementType(Block parent) {
        ...
        ASTBlock it = ObjectUtil.tryCast(parent, ASTBlock.class);
        if (it == null) {
            return null;
        }
    ...
    @Nullable
    Alignment computeAlignment(@Nonnull ASTNode node) {
    ...
    @Nullable
    Indent computeBlockIndent(@Nonnull ASTNode node) {
    ...
    private @Nullable Indent computeTemplateIndent(TextRange nodeTextRange) {
        ...
        if (!StringUtil.isEmptyOrSpaces(document.getCharsSequence().subSequence(lineStartOffset, nodeTextRange.getStartOffset()))) {
            return null;
        }

        return Formatter.getInstance().getIndent(Indent.Type.SPACES, true, nodeTextRange.getStartOffset() - lineStartOffset, false, false);
    }

    @Nullable
    Indent computeNewChildIndent(@Nonnull ASTNode node) {
    ...
    public boolean isIncomplete(@Nonnull ASTNode node) {
    ...
    private static @Nullable Indent computeKeyValuePairIndent(@Nonnull ASTNode node) {
        IElementType parentType = PsiUtilCore.getElementType(node.getTreeParent());
        IElementType grandParentType = parentType == null ? null : PsiUtilCore.getElementType(node.getTreeParent().getTreeParent());
    ...
    private @Nonnull Indent computeSequenceItemIndent(@Nonnull ASTNode node) {
        IElementType parentType = PsiUtilCore.getElementType(node.getTreeParent());
        IElementType grandParentType = parentType == null ? null : PsiUtilCore.getElementType(node.getTreeParent().getTreeParent());
        boolean grandParentIsDocument = grandParentType == YAMLElementTypes.DOCUMENT;
