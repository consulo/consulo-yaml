...
public abstract class YamlComposedTypeBase extends YamlMetaType {
  ...
  protected YamlComposedTypeBase(@Nonnull String typeName, @Nonnull String displayName, List<YamlMetaType> types) {
  ...
  private static <T> List<T> copyList(@Nonnull List<T> list) {
    return list.isEmpty() ? Collections.emptyList() : new ArrayList<>(list);
  }

  private @Nullable Field mergeFields(@Nonnull List<Pair<Field, YamlMetaType>> pairs) {
    switch (pairs.size()) {
      case 0 -> {
        return null;
      }
  ...
  private static boolean mergeIsMany(@Nonnull String name, @Nonnull List<Pair<Field, YamlMetaType>> fields) {
  ...
  protected static ProblemsHolder makeCopy(@Nonnull ProblemsHolder original) {
    return InspectionManager.getInstance(original.getProject()).createProblemsHolder(original.getFile(), original.isOnTheFly());
  }
}
